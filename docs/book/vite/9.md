...
[Esbuild](https://esbuild.github.io/) 是由 Figma 的 CTO 「Evan Wallace」基于 Golang 开发的一款打包工具，相比传统的打包工具，主打性能优势，在构建速度上可以比传统工具快 `10~100` 倍。那么，它是如何达到这样超高的构建性能的呢？主要原因可以概括为 4 点:
...
async function runBuild() {
  ...
  const result = await build({
    ...
    // 🚨 原小册中是有这个配置的，但当前esbuild v0.17.6 版本已移除该配置项
    // watch: false,
    // 是否将产物写入磁盘
    write: true,
...
在项目打包方面，除了`build`和`buildSync`，Esbuild 还提供了另外一个比较强大的 API——`serve`。这个 API有3个特点:
...
下面，我们通过一个具体例子来感受一下, 可参考 [esbuild#serve](https://esbuild.github.io/api/#serve)。
...
```js
// 🚨 Esbuild v0.17.6版本中已经不存在 `serve` 接口了，下面代码为之前0.14版本示例。
// build.js
...
运行 `node build.mjs`，我们在浏览器访问`localhost:8000/index.js`可以看到 Esbuild 服务器返回的编译产物如下所示：

![esbuild serve](./imgs/9-4.webp)
...
插件开发其实就是基于原有的体系结构中进行`扩展`和`自定义`。 [Esbuild插件](https://esbuild.github.io/plugins/#using-plugins)也不例外，通过 Esbuild 插件我们可以扩展 Esbuild 原有的路径解析、模块加载等方面的能力，并在 Esbuild 的构建过程中执行一系列自定义的逻辑。
...
在 Esbuild 插件中，[onResolve](https://esbuild.github.io/plugins/#on-resolve) 和 [onLoad ](https://esbuild.github.io/plugins/#on-load)是两个非常重要的钩子，分别控制路径解析和模块内容加载的过程。
...
![esbuild cdn plugin](./imgs/9-6.webp)
...
![esbuild cdn plugin](./imgs/9-7.webp)
...
![esbuild路径解析](./imgs/9-8.webp)

现在我们再次执行`node build.js`，发现依赖已经成功下载并打包了。

![esbuild build](./imgs/9-9.webp)
...
![serve](./imgs/9-10.webp)

再访问`localhost:3000`，会默认访问到 index.html 的内容：

![serve success](./imgs/9-11.webp)
