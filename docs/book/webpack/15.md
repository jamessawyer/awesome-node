...
其次，新版本通常还会引入更多性能工具，例如 Webpack5 的 [cache](./13)(持久化缓存)、[lazyCompilation](https://webpack.js.org/configuration/experiments/#experimentslazycompilation)（按需编译，下面展开介绍） 等。因此，开发者应该保持时刻更新 Webpack 以及 Node、NPM or Yarn 等基础环境，尽量使用最新稳定版本完成构建工作。
...
module.exports = {
  // ...
  experimental: { // [!code focus]
    lazyCompilation: true // [!code focus]
  }, // [!code focus]
}
```
    ...
    rules: [
      {
        test: /\.js$/,
        exclude: { // [!code focus]
          and: [/node_modules/], // [!code focus]
          not: [/node_modules\/lodash/] // [!code focus]
        }, // [!code focus]
        use: ['babel-loader', 'eslint-loader'],
      }
    ]
...
- [Rule.exclude Condition - webpack docs](https://webpack.docschina.org/configuration/module/#condition)
...
对我们来说，这些资源文件都是独立、内聚的代码片段，没必要重复做代码解析、依赖分析、转译等操作，此时可以使用 [module.noParse](https://webpack.docschina.org/configuration/module/#modulenoparse) 配置项跳过这些资源，例如：
...
🚀此时，真正有效的代码被包含在 `react.development.js`（或 `react.production.min.js`）中，但 Webpack 只会打包这段 `index.js` 内容，也就造成了产物中实际上并没有真正包含 React。针对这个问题，我们可以先找到适用的代码文件，然后用 [resolve.alias](https://webpack.docschina.org/configuration/resolve/#resolvealias) 配置项重定向到该文件：
...
使用 [externals](./18#使用外置依赖) 也能将部分依赖放到构建体系之外，实现与 `noParse` 类似的效果，详情可查阅[externals - webpack doc](https://webpack.docschina.org/configuration/externals/)。
...
针对 `source-map` 功能，Webpack 提供了 [devtool](https://webpack.docschina.org/configuration/devtool/) 选项，可以配置 `eval`、`source-map`、`cheap-source-map` 等值，不考虑其它因素的情况下，最佳实践：
...
![webpack sourcemap](./imgs/15-1.webp)

不同于小册作者的想法：

- 开发环境使用 `cheap-module-eval-source-map` 确保经过各种loaders转换之后的代码与源文件对应上
- 生产环境使用 `none` 或者 `nosources-source-map`

关于sourcemap可以看：

- [汪磊webpack devtools - @bilibili](https://www.bilibili.com/video/BV1kP41177wp?p=48&vd_source=1b80f8e85d6b313a57c6e37edadd9d91)
...
需要注意，这类增强资源搜索体验的特性背后涉及许多 IO 操作，本身可能引起较大的性能消耗，开发者可根据实际情况调整 [resolve](https://webpack.docschina.org/configuration/resolve/) 配置，缩小资源搜索范围，包括：
...
Webpack 可以通过一些优化措施来提高构建速度和性能，以下是一些常见的优化措施：

1. **减少文件搜索范围**

Webpack 在构建过程中需要搜索项目中所有依赖的文件，这个过程会比较耗时。可以通过配置 `resolve.modules` 和 `resolve.extensions` 来缩小搜索范围，加快构建速度。

2. **使用 DllPlugin 和 DllReferencePlugin**

DllPlugin 和 DllReferencePlugin 是两个用于拆分和预编译公共模块的插件。将一些常用的库（如 React、lodash 等）打包成单独的 DLL 文件后，在每次重新构建应用程序时，只需要加载 DLL 文件，而不需要重复编译公共模块，可以极大地提高构建速度。

3. **利用缓存**

Webpack 的缓存可以减少重复构建，提高构建速度。可以使用webpack5中的 [cache](./13) 功能

4. **使用 HappyPack**

HappyPack 是一个多线程构建工具，可以将 loader 的执行过程放在多个子进程中进行，从而提高构建速度。

5. **利用 Tree-Shaking**

Tree-Shaking 是 Webpack 4 中新增的特性，可以将未被使用的代码从最终打包的文件中剔除掉，从而减少文件大小和加载时间。

6. **代码分离**

Webpack 支持将代码拆分成多个 chunk，在需要时动态加载。可以使用 [SplitChunksPlugin](./16) 来实现代码的拆分，从而加快页面的加载速度。

7. **压缩文件**

使用 UglifyJsPlugin 和 OptimizeCSSAssetsPlugin 等插件可以对 JavaScript 和 CSS 文件进行压缩，减少文件大小，提高加载速度。

总的来说，以上优化措施都可以在不同程度上提高 Webpack 的构建速度和性能。需要根据具体项目的需求选择相应的优化方案。
