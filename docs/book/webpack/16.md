...
- [webpack SplitChunkPlugin doc](https://webpack.docschina.org/plugins/split-chunks-plugin/)
...
- 分配完毕后，根据 SplitChunksPlugin 的启发式算法进一步对这些 Chunk 执行**裁剪、拆分、合并、代码调优**，最终调整成运行性能(可能)更优的形态；
- 最后，将这些 Chunk 一个个输出成最终的产物(Asset)文件，编译工作到此结束。

![modules to chunks](./imgs/16-1.webp)
...
3. `Runtime Chunk`：运行时代码抽离成 Runtime Chunk，可通过 [entry.runtime](https://webpack.js.org/configuration/entry-context/#dependencies) 配置项实现。
...
由于 Webpack4 开始已经内置支持 `SplitChunksPlugin` ，我们不需要额外安装依赖，直接修改 [optimization.splitChunks](https://webpack.docschina.org/plugins/split-chunks-plugin/#optimizationsplitchunks) 配置项即可实现自定义的分包策略：
...
::: details Claude关于minSize/minChunks/maxInitialRequest解释
...
这三个规则的关系可以描述为:**Extractor = minSize <= chunkSize <= maxSize && occurrences >= minChunks**也就是当模块体积在 minSize 和 maxSize 范围内,且其出现次数不小于 minChunks 时,才会被 SplitChunksPlugin 提取到缓存组中。

通过均衡地配置这三个值,可以实现:

1. 避免请求过多导致的性能损耗;
2. 提取出合理体积的 chunk,既不会文件过小导致的请求浪费,也不会文件过大导致的加载慢;
3. 选择出现频率高的模块进行提取,获得最大的优化效果。这三个规则的灵活运用,是使用 SplitChunksPlugin 优化代码的关键。理解每个规则的作用及其之间的关系,可以更好地配置出满足需求的拆分结果。
...
- 🌰 [splitChunks-basic - @github](https://github1s.com/Tecvan-fe/webpack-book-samples/blob/main/splitchunks-basic/webpack.config.js)
...
通过 [emitAssets](https://webpack.js.org/api/compilation-object/#emitasset) 等方式直接输出产物文件不在此范畴。

:::

1️⃣ 举个例子，对于上例所说的模块关系：

![modules关系](./imgs/16-5.webp)
...
- `maxSize`： 超过这个尺寸的 Chunk 会尝试**进一步**拆分出更小的 Chunk；
...
上述 `minChunks`、`maxInitialRequests`、`minSize` 都属于**分包条件**，决定是否对什么情况下对那些 Module 做分包处理。此外， `SplitChunksPlugin` 还提供了 `cacheGroups` 配置项用于为不同文件组设置不同的规则，例如：
...
::: details SplitChunksPlugin实现chunks拆分的原理
1. 分析模块之间的依赖关系,找到可以被提取出来的模块。
2. 根据配置的规则,决定这些模块应该放到哪个缓存组中。
3. 根据缓存组的配置,将这些模块提取出来,形成一个单独的 chunk。
4. 将原来的 chunks 中的模块替换为该 chunk 的引用。
5. 在页面中加载这个新生成的 chunk。

具体的实现过程如下:
1. 在编译阶段的优化钩子函数中,对所有模块及其依赖进行深度优先遍历。
2. 根据 minSize、maxSize 和 minChunks 等规则过滤掉不符合条件的模块。
3. 对符合条件的模块,根据 module 和 chunks 的名称、来源等属性决定它属于哪个缓存组。每个模块只能属于一个缓存组。
4. 遍历所有的缓存组,根据每个组的设置,决定从哪些 chunk 中提取模块到该组中。
5. 将选出的模块移动到该缓存组的 chunk 中。
6. 更新原 chunks 中对这些模块的引用,指向新的 chunk。
7. 为新生成的 chunk 配置加载规则,在页面加载时请求该 chunk。
8. 最终页面上加载的不是许多小的重复 chunk,而是一些更大的共用 chunk。

所以,SplitChunksPlugin 的原理是通过分析模块关系,提取公共模块到更大的 chunk 中,减小重复模块的资源请求次数,提高缓存利用效率,优化页面加载性能。这是它实现代码拆分的核心原理。
:::

扩展阅读：
  - [🔥Webpack: An in-depth introduction to SplitChunksPlugin Webpack：SplitChunksPlugin - @indepth.dev](https://indepth.dev/posts/1490/webpack-an-in-depth-introduction-to-splitchunksplugin) 理解如何分包必读🚀
